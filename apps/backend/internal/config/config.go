...
func LoadConfig() (*Config, error) {
	...
	for _, env := range os.Environ() {
		...
		if len(parts) == 2 && strings.HasPrefix(parts[0], "taskgo_") {
			key := parts[0]
			value := parts[1]

			configKey := strings.ToLower(strings.TrimPrefix(key, "taskgo_"))
	...
	err := k.Load(env.ProviderWithValue("taskgo_", ".", func(key, value string) (string, any) {
		return strings.ToLower(strings.TrimPrefix(key, "taskgo_")), value
	}), nil)
	if err != nil {
	...
	mainConfig.Observability.ServiceName = "taskgo"
